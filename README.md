...
- The function initially sets the left voltage limit for the linear fit at `Vgmax` (or `Vgmax+offset` - we will mention the `offset` later) and the right voltage limit at a distance of `minRangeLength` volts away (usually 20 V). The right voltage limit is at a gate voltage `Vg(i)`, where `i` is the corresponding row number. `i` is larger than `maxrows/2` because we are fitting in the reverse sweep. A linear fit is performed at that range, and the value of the slope is stored inside the variable `firstfit`, since it is the first linear fit performed.
